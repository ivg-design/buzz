...
///    exist for ordinary channels. A DM is already addressed to every member,
///    so DM events satisfy this condition without a redundant `p` tag. Tag kind
///    is checked via `tag.as_slice()` for stable, library-independent access.
...
mod tests {
    ...
    async fn test_match_event_require_mention() {
        ...
        // A DM is already addressed to every participant. The same untagged
        // event must pass the production rule matcher when its channel is a DM.
        let matched = match_event(&event_no_mention, channel_id, &rules, agent_pubkey, true)
            .await
            .expect("an untagged DM message is implicitly addressed");
        assert_eq!(matched.prompt_tag, "mentioned");
    }
