...
pub async fn run(state: &SharedState, p: ViewImageParams) -> Result<CallToolResult, ErrorData> {
    ...
    prepare_result(&raw, max_dim, &source_label)
}

/// Validate, decode, resize, and encode bytes supplied by a trusted transport.
/// The same pixel, allocation, dimension, animation, and output-size limits as
/// the generic `view_image` tool apply.
pub(crate) fn prepare_result(
    raw: &[u8],
    max_dim: u32,
    source_label: &str,
) -> Result<CallToolResult, ErrorData> {
    let max_dim = max_dim.clamp(MIN_MAX_DIM, MAX_MAX_DIM);
...
/// Authenticated relay media is handled only by the harness-owned typed MCP.
async fn fetch_url(url: &str) -> Result<Vec<u8>, ErrorData> {
    ...
    if !resp.status().is_success() {
        ...
        if matches!(status.as_u16(), 401 | 403) {
            return Err(invalid_params(format!(
                "fetch {url} returned HTTP {status}; use buzz_private_media_get for relay-private images"
            )));
        }
        return Err(invalid_params(format!(
